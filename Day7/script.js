...
const parse_tree = (input) => {
  const filesystem = { parent: null, folders: {}, files: {}, size: 0 };

  let current_folder = filesystem;
    ...
    .forEach((line) => {
      if (line.includes("$")) {
        ...
        switch (command) {
          case "cd":
            switch (argument) {
              ...
              default:
                const target_folder = current_folder.folders[argument];
                if (!target_folder)
                  throw `cd mismatch: ${current_folder} => /${argument}`;
        ...
        if (isNaN(+item1)) {
          ...
          current_folder.folders[dir] = new_folder;
        } else {
          const [size, filename] = [item1, item2];
          current_folder.files[filename] = size;

          current_folder.size += +size;
          let parent = current_folder.parent;
          while (true) {
            if (!parent) break;
            parent.size += +size;
...
console.assert(
  test_tree_1.folders.a.size == 94853,
  "The directory a has total size 94853 because it contains files f (size 29116), g (size 2557), and h.lst (size 62596), plus file i indirectly"
);
console.assert(
  test_tree_1.folders.d.size == 24933642,
  "Directory d has total size 24933642."
);
console.assert(
  test_tree_1.size == 48381165,
